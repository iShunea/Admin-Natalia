...
function ReactTable({ columns, data }) {
  ...
  return (
    <MainCard
      title={matchDownSM ? 'Testimonials' : 'Testimonials Table'}
      content={false}
      secondary={
...
export default function SortingTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const retrieveTestimonials = await axiosInstance.get('admin/testimonials');
        if (retrieveTestimonials.status === 200) {
          setData(retrieveTestimonials.data);
        } else {
          console.error('Failed to retrieve testimonials');
        }
      } catch (error) {
